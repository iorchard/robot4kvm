#!/bin/bash
# vm_man4ubuntu.sh: VM image manipulation script.
#
...
# packages to install
IPKGS="less,sudo,vim,man-db,dnsutils,telnet,curl,sshpass,git,cloud-initramfs-growroot"
# packages to remove
...
# Create /tmp/netcfg.yaml
cat data/netcfg.yaml.prefix /tmp/eth* > /tmp/netcfg.yaml
...
$VIRTC -a ${IMGFILE} \
    ...
    --upload data/resolved.conf:/etc/systemd/resolved.conf \
    --upload /tmp/netcfg.yaml:/etc/netplan/netcfg.yaml \
